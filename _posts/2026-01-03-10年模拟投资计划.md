...
| **防守-对冲** | **0-3年国开债** | `511360` | **20%** | 200,000 | 稳定收益 / 低波动防守 |
...
* **黄金踏空风险**：将黄金改为定投后，若下个月发生地缘黑天鹅导致金价暴涨，你可能只有 1/10 的仓位在车上。这是为了平稳而付出的代价（但黄金仍按 1.5w/月持续定投跟进，且组合本身股·债·金三元分散，此风险可控）。

## 5. 部署自动化程序

> 注：实际仓库 `permanent-portfolio-monitor` 现已托管三套相互独立的策略（Hi5 美元实盘 / 40-60 人民币实盘 / 本 100 万模拟盘）。本节只讲本模拟盘（`monitor_sim100w.py`）的部署——它的行情走新浪财经，**无需任何 Cookie**。
...
2.  Repository name 填 `permanent-portfolio-monitor`，勾选 `Private`（私有，保护你的隐私），点击 Create。

### 配置密钥 (Secrets)

为了不把你的邮箱授权码明文写在代码里，我们需要用 GitHub Secrets。

1.  在你的仓库页面，点击上方的 **Settings** -> 左侧侧边栏 **Secrets and variables** -> **Actions**。

2.  点击 **New repository secret**，依次添加以下 2 个变量（本模拟盘行情走新浪财经，无需 Cookie）：
...
3.  将文件名 `main.yml` 改为 `sim100w_check.yml`。
...
on:
  schedule:
    # 北京时间每天 09:30 运行 (UTC 01:30)，A 股盘中检查
    - cron: '30 1 * * *'
  workflow_dispatch:
...
PORTFOLIO_CONFIG = {
    ...
    "sh511360": {"target": 0.20, "base_price": 113.019, "name": "0-3年国开债"},
